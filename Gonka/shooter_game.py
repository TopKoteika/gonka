...
bg = image.load('doroga.png')
bg = transform.scale(bg, (WIDTH, HEIGHT))
bg_y1 = 0
bg_y2 = -HEIGHT

player_img = image.load("car-removebg-preview.png")
enemy_img = image.load("Police.png")
...
class Player(Sprite):
    ...
    def update(self):
        ...
        for enemy in enemys:
            if self.rect.y < enemy.rect.y and not enemy.overtaken:
...
player = Player(player_img, 150, 150, 300, 300)
enemys = sprite.Group()
enemy1 = Enemy(enemy_img, 150, 150)
...
start_screen = True
start_text = font2.render(f"F TO START", True, (255, 255, 255))
    ...
    if start_screen:
        ...
        if now - enemy_spawn_time > spawn_interval:  # якщо від появи останнього ворога пройшло більше 1с
            ...
            for i in range(rand_k):
                enemy1 = Enemy(enemy_img, 150, 150)  # створюємо нового ворога
